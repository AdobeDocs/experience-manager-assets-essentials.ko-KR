...
| 링크 | 을(를) 추가하여 외부 URL을 활성화합니다. 메타데이터 양식에 URL 구성 요소를 구성하면 메타데이터 편집기 페이지에서 특정 메타데이터 속성에 매핑할 수 있습니다. 이 [메타데이터 양식은 폴더에 할당할 수 있습니다](#assign-metadata-form-folder). |
...
링크 구성 요소는 스토리지 링크, 저작권 정보, 연락처 양식 등을 포함하는 외부 URL을 활성화하는 데 사용됩니다. 메타데이터 양식에서 링크 구성 요소를 사용하려면 [메타데이터 양식을 구성](#metadata-forms)해야 합니다. URL 구성 요소는 메타데이터 편집기 페이지에서 특정 메타데이터 속성에 매핑될 수 있으며, 이 속성은 다른 메타데이터 구성 요소와 동일한 방식으로 작동합니다. 이 메타데이터 양식은 폴더에 할당할 수 있습니다. 따라서 할당된 폴더 자산의 세부 정보 페이지에 자산 링크 메타데이터 값이 표시될 수 있습니다.
