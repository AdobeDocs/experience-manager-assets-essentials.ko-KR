...
* 계층적 태그는 유연하며 용어를 논리적인 방법으로 구성하는 훌륭한 방법입니다. 네임스페이스, 태그 및 서브태그를 통해 전체 분류 체계를 표현할 수 있다.

* 태그는 시간이 지나면서 조직의 어휘가 변경됨에 따라 발전할 수 있습니다.

* Assets에서 as a Cloud Service으로 관리되는 태그는 Assets Essentials에서 관리되는 태그와 계속 동기화되므로 메타데이터 거버넌스와 무결성을 보장합니다.

에셋에 태그를 적용하려면 먼저 네임스페이스를 만든 다음 태그를 만들어 추가해야 합니다. 태그를 만들어 기존 네임스페이스에 추가할 수도 있습니다. 루트 수준에서 만드는 모든 태그는 표준 태그 네임스페이스에 자동으로 추가됩니다. 그런 다음 태그 필드를 메타데이터 양식에 추가하여 자산 세부 정보 페이지에 표시할 수 있습니다. 이러한 설정을 구성하고 나면 자산에 태그 적용을 시작할 수 있습니다.
...
[!DNL Assets Essentials] 저장소에 업로드할 때 에셋에 스마트 태그로 추가해서는 안 되는 단어를 포함하는 차단 목록에 추가하다를 구성할 수 있습니다. 이 기능은 브랜드 규정 준수를 유지하는 데 도움이 되며 스마트 태그를 조정해야 하는 번거로움을 제거해 줍니다.
...
[!DNL Assets Essentials] 관리자가 기존 및 새로 추가된 에셋에 대한 스마트 태그를 차단할 수 있습니다. 태그를 차단하려면 다음 단계를 수행하십시오.
...
Assets Essentials을 사용하면 나열된 차단된 태그를 CSV 형식으로 내보낼 수 있습니다. 차단 목록에 추가하다를 내보내려면 아래 단계를 수행합니다.
...
Assets Essentials은 데이터 소스(CSV)에서 차단된 태그를 가져오는 기능을 제공합니다. 차단 목록에 추가하다를 가져오려면 아래 단계를 수행합니다.
