...
Assets Essentials의 최신 릴리스는 2023년 5월 24일에 출시되었습니다.

최근에 추가된 기능 목록은 다음과 같습니다.

**가장 많이 다운로드한 자산**

이제 내 작업 공간에 Assets Essentials 환경에 대해 가장 많이 다운로드된 상위 10개의 자산이 표시됩니다. [!UICONTROL 콘텐츠] 섹션. 나열된 각 에셋에 대한 형식 유형 및 다운로드 수를 볼 수도 있습니다.

**에셋 메타데이터에 대한 일괄 업데이트**

일괄 메타데이터 업데이트를 통해 사용자는 여러 에셋에서 공통 메타데이터 업데이트를 동시에 수행할 수 있습니다. 레코드를 개별적으로 업데이트할 필요가 없으며 검색을 통해 액세스한 에셋 또는 폴더에 속성을 빠르게 적용할 수 있습니다.
...
내 작업 영역에는 인사이트, 작업 및 컨텐츠에 대한 위젯이 포함됩니다. 환경 설정에 따라 이러한 위젯이 작업 영역에 표시되는 방식을 구성할 수 있습니다.
...
이제 Assets Essentials는 Assets Essentials 사용자 인터페이스를 사용하여 에셋을 공유하도록 선택하는 즉시 자동으로 링크를 생성합니다. 생성된 링크는 만료일을 변경해도 유효합니다.
...
* [!DNL Settings] 아래 [!UICONTROL 메타데이터 양식] 옵션의 에셋 세부 정보 화면에서 사용자에게 메타데이터 속성을 표시하는 데 사용할 사용자 정의 메타데이터 양식을 만들고 관리할 수 있습니다. [메타데이터 양식](metadata.md#metadata-forms)을 참조하십시오.
